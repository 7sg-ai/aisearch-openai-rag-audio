...
def attach_rag_tools_to_mini(
    ...
    search_client = SearchClient(
        search_endpoint, search_index, credentials, user_agent="MiniAPI"
    )

    mini_api.add_tool(
        "search",
        _search_tool_schema,
        lambda args: _search_tool(
            search_client,
    ...
    mini_api.add_tool(
        "report_grounding",
        _grounding_tool_schema,
        lambda args: _report_grounding_tool(
            search_client, identifier_field, title_field, content_field, args
        ),
    )
