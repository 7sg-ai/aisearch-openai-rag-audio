...
async def create_app():
    ...
    realtime_deployment = os.environ.get(
        "AZURE_OPENAI_REALTIME_DEPLOYMENT", "gpt-realtime-mini"
    )
    voice_choice = os.environ.get("AZURE_OPENAI_REALTIME_VOICE_CHOICE") or "alloy"

    logger.info(
        f"[App] Initializing RTMiddleTier with endpoint: {os.environ['AZURE_OPENAI_ENDPOINT']}"
    )
    logger.info(
        f"[App] Realtime deployment: {realtime_deployment}, Voice: {voice_choice}"
    )
    rtmt = RTMiddleTier(
    ...
    if os.environ.get("AZURE_STORAGE_ENDPOINT") and os.environ.get(
        "AZURE_SEARCH_INDEX"
    ):
        try:
            sync_interval = int(
                os.environ.get("DOCUMENT_SYNC_INTERVAL_SECONDS", "300")
            )  # Default: 5 minutes
