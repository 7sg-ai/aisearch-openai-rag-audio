...
class DocumentSync:
    ...
    async def sync_documents(self) -> None:
        ...
        try:
            if not self.data_path.exists():
                logger.debug(
                    f"Data folder does not exist: {self.data_path} (this is normal in production)"
                )
                return

            if not self.data_path.is_dir():
                logger.warning(
                    f"Data path exists but is not a directory: {self.data_path}"
                )
                return
            ...
            existing_blobs = {
                blob.name: blob.last_modified for blob in container_client.list_blobs()
            }
                ...
                if file_path.is_file() and not file_path.name.startswith("."):
                    ...
                    if filename not in existing_blobs:
                        ...
                        if local_mtime > blob_mtime:
                            logger.info(
                                f"File updated locally, re-uploading: {filename}"
                            )
                            should_upload = True
    ...
    async def _sync_loop(self) -> None:
        ...
        logger.info(
            f"Document sync started (checking every {self.sync_interval_seconds} seconds)"
        )
        while self._running:
            try:
