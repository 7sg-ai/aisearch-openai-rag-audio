...
class MiniAPI:
    ...
    def _pcm_to_wav(
        ...
        wav_header += struct.pack(
            "<I", sample_rate * channels * sample_width
        )  # byte rate
    ...
    async def transcribe_audio(self, audio_data: bytes) -> str:
        ...
        try:
            logger.info(
                f"[MiniAPI] Starting transcription, audio size: {len(audio_data)} bytes"
            )
            logger.info(
                f"[MiniAPI] Using endpoint: {self.endpoint}, deployment: {self.realtime_deployment}"
            )
            ...
            try:
                ...
                logger.error(
                    f"[MiniAPI] Error converting PCM to WAV: {wav_error}", exc_info=True
                )
                raise web.HTTPInternalServerError(
                    text=f"Audio conversion failed: {str(wav_error)}"
                )
            ...
            logger.info(
                f"[MiniAPI] Calling transcription API with model: {self.realtime_deployment}"
            )
            try:
                ...
                logger.error(
                    f"[MiniAPI] Transcription API error ({error_type}): {api_error}",
                    exc_info=True,
                )
                # Check for common errors
                if (
                    "deployment" in str(api_error).lower()
                    or "model" in str(api_error).lower()
                ):
                    raise web.HTTPInternalServerError(
                    ...
                    "authentication" in str(api_error).lower()
                    or "unauthorized" in str(api_error).lower()
                ):
                    raise web.HTTPInternalServerError(
                    ...
                    raise web.HTTPInternalServerError(
                        text=f"Transcription API error: {str(api_error)}"
                    )
            ...
            logger.error(
                f"[MiniAPI] Unexpected error transcribing audio: {e}", exc_info=True
            )
    ...
    async def chat_completion(self, user_message: str) -> dict:
        ...
        try:
            logger.info(
                f"[MiniAPI] Starting chat completion, user message: {user_message}"
            )
            ...
            if self.tools:
                ...
                logger.info(
                    f"[MiniAPI] Using {len(tools)} tools: {list(self.tools.keys())}"
                )

            # Call chat completion API
            logger.info(
                f"[MiniAPI] Calling chat completion API with model: {self.chat_deployment}"
            )
            ...
            if assistant_message.tool_calls:
                logger.info(
                    f"[MiniAPI] Assistant requested {len(assistant_message.tool_calls)} tool calls"
                )
                    ...
                    if tool_name in self.tools:
                        ...
                        if hasattr(tool_result, "destination"):
                            ...
                            if tool_result.destination == ToolResultDirection.TO_CLIENT:
                                ...
                                if hasattr(tool_result, "text") and isinstance(
                                    tool_result.text, dict
                                ):
                ...
                assistant_message = response.choices[0].message
                logger.info(
                    f"[MiniAPI] Final response received: {assistant_message.content}"
                )
    ...
    async def synthesize_speech(self, text: str) -> bytes:
        ...
        try:
            ...
            logger.info(
                f"[MiniAPI] Using TTS model: {self.tts_deployment}, voice: {self.voice_choice}"
            )
            ...
            logger.info(
                f"[MiniAPI] Speech synthesis successful, audio size: {len(audio_data)} bytes, chunks: {chunk_count}"
            )
            return audio_data
    ...
    async def transcribe_handler(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"[MiniAPI] Decoded base64, audio size: {len(audio_data)} bytes"
                )
            except Exception as decode_error:
                logger.warn(
                    f"[MiniAPI] Base64 decode failed: {decode_error}, using data as-is"
                )
                audio_data = data
            ...
            logger.error(
                f"[MiniAPI] Transcribe handler error ({error_type}): {error_msg}",
                exc_info=True,
            )
            # Return detailed error for debugging
            return web.json_response(
                {"error": error_msg, "error_type": error_type, "details": str(e)},
                status=500,
            )
    ...
    async def synthesize_handler(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            logger.info(
                f"[MiniAPI] /synthesize endpoint returning audio, size: {len(audio_base64)} chars"
            )
