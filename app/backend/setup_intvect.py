...
def setup_index(
    ...
    if index_name in index_names:
        logger.info(
            f"Index {index_name} already exists, checking if it needs updates..."
        )
        ...
        if "parent_id" not in existing_field_names:
            logger.warning(
                f"Index {index_name} is missing 'parent_id' field required for index projections. Index will be recreated."
            )
    ...
    if skillset_exists and index_needs_update:
        logger.info(
            f"Deleting existing skillset {index_name} to recreate with updated index schema..."
        )
        try:
    ...
    if index_needs_update:
        ...
        for indexer in indexers:
            if indexer.target_index_name == index_name:
                logger.info(
                    f"Deleting indexer {indexer.name} because index was recreated..."
                )
                try:
    ...
    schedule_interval_minutes = int(
        os.environ.get("AZURE_SEARCH_INDEXER_SCHEDULE_MINUTES", "60")
    )
    ...
    if indexer_exists:
        ...
        try:
            ...
            if full_indexer.schedule is None:
                logger.info(
                    f"Adding schedule to existing indexer (runs every {schedule_interval_minutes} minutes)"
                )
                full_indexer.schedule = IndexingSchedule(
                    interval=schedule_interval_iso8601
                )
                indexer_client.create_or_update_indexer(full_indexer)
            else:
                logger.info(
                    f"Indexer already has a schedule: {full_indexer.schedule.interval}"
                )
        except Exception as e:
            logger.warning(f"Could not update indexer schedule: {e}")
    else:
        logger.info(
            f"Creating indexer: {index_name} with schedule (runs every {schedule_interval_minutes} minutes)"
        )
